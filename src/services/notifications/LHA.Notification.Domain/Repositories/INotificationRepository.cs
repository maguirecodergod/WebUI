...
public interface INotificationRepository : IRepository<NotificationEntity, Guid>
{
    ...
    Task<IEnumerable<NotificationEntity>> GetByCorrelationIdAsync(Guid correlationId, CancellationToken cancellationToken = default);
    ...
    Task<int> GetUnreadCountByTenantAsync(CancellationToken cancellationToken = default);
    Task<int> GetUnreadCountByRecipientAsync(Guid recipientId, CancellationToken cancellationToken = default);
}