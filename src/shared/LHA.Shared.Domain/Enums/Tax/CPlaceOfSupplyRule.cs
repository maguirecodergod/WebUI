namespace LHA.Shared.Domain
{
    /// <summary>
    /// Place-of-supply rule that determined the taxing jurisdiction.
    /// Audit trail for which rule was applied at the time of supply.
    /// </summary>
    public enum CPlaceOfSupplyRule
    {
        /// <summary>
        /// Default for physical goods
        /// </summary>
        SupplierLocation = 1,
        /// <summary>
        /// Digital services (EU VAT 2015, OECD)
        /// </summary>
        CustomerLocation = 2,
        /// <summary>
        /// Restaurant, live events
        /// </summary>
        ServicePerformed = 3,
        /// <summary>
        /// Real estate
        /// </summary>
        PropertyLocation = 4,
        /// <summary>
        /// Passenger transport
        /// </summary>
        DeparturePoint = 5,
        /// <summary>
        /// EU One-Stop-Shop for B2C digital below threshold
        /// </summary>
        OSSRegistration = 6,
        /// <summary>
        /// Where the customer has a fixed establishment
        /// </summary>
        FixedEstablishment = 7
    }
}