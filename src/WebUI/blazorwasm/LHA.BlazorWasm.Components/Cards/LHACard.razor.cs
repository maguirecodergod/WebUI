...
namespace LHA.BlazorWasm.Components.Cards
{
    public partial class LHACard : LHAComponentBase
    {
        private readonly string _titleId = $"lha-card-title-{Guid.NewGuid():N}";
        ...
        {
            var sb = new StringBuilder("lha-card");

            if (Bordered) sb.Append(" lha-card--bordered");
            if (Hoverable) sb.Append(" lha-card--hoverable");
            if (Clickable) sb.Append(" lha-card--clickable");
            if (Loading) sb.Append(" lha-card--loading");
            if (Grid) sb.Append(" lha-card--grid");
